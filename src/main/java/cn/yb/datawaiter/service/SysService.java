...
public class SysService implements ISysService {
    ...
    public ApiEntity findApiByURL(String relativeURL) {
        String sql = "SELECT * FROM "+AnnotationTool.getTableName(ApiEntity.class)  +" WHERE  Concat(Concat(rooturl , '/'), selfURL) ="+ JDBCUtils.sqlStr(relativeURL);
    ...
    public ApiEntity findApiByURLAndProjectid(String relativeURL, String projectid) {
        ...
        String sql = "SELECT * FROM "+ AnnotationTool.getTableName(ApiEntity.class)+" WHERE  projectid= '"+projectid+"' AND Concat(Concat(rooturl , '/'), selfURL) ="+ JDBCUtils.sqlStr(relativeURL);
        List<JSONObject> jsons = Select.findDataBySQL(SysConn,sql);
        if(Tool.isEmpty(jsons)){
            return  null;
        }
